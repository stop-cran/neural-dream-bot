...
from scipy.misc import imresize, fromimage, toimage
...
def gs_weights_read(url, cache_path):
    if os.path.isfile(cache_path):
        print("Model weights from bucket", url_parts.netloc, "path:", path, "url:", url, "are already loaded.")
    else:
        url_parts = urlparse(url)
        path = url_parts.path[1:]
        assert url_parts.scheme == 'gs'
        print("Reading model weights from bucket", url_parts.netloc, "path:", path, "url:", url)

        blob = storage.Client() \
            .get_bucket(url_parts.netloc) \
            .get_blob(path)

        if blob is None:
            return None
            
        print("Caching model weights to", cache_path)
        
        with open(cache_path, 'wb') as f:
            blob.download_to_file(f)
    
def gsimread(url, mode):
    url_parts = urlparse(url)
    path = url_parts.path[1:]
    assert url_parts.scheme == 'gs'
    print("Reading image from bucket", url_parts.netloc, "path:", path, "url:", url)
    ...
    with BytesIO() as f:
        blob.download_to_file(f)
        f.seek(0)
        image=imread(f, pilmode=mode)
    return image

def gsimwrite(image, url):
    url_parts = urlparse(url)
    path = url_parts.path[1:]
    assert url_parts.scheme == 'gs'
    print("Writing image to bucket", url_parts.netloc, "path:", path, "url:", url)

    with BytesIO() as f:
        print("Writing image to memory file...")
        imsave(f, image, format='jpg')
        f.seek(0)
        print("Creating blob...")
        blob = storage.Client() \
            .get_bucket(url_parts.netloc) \
...
TF_WEIGHTS_PATH_NO_TOP = 'gs://neuralstylebot.appspot.com/models/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5'
TF_19_WEIGHTS_PATH_NO_TOP = 'gs://neuralstylebot.appspot.com/models/vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5'


parser = argparse.ArgumentParser(description='Neural style transfer with Keras.')
parser.add_argument('--content_image', type=str,
                    help='A relatiuve path to the image to transform.')

parser.add_argument('--syle_images', nargs='+', type=str,
                    help='Path to the style reference image.')

parser.add_argument('--job-dir', type=str, default=None,
                    help='Job directory for the saved results.')

parser.add_argument('--callback_url', type=str, default=None,
                    help='URL for a task competion callback.')
...
parser.add_argument("--num_iter", dest="num_iter", default=5, type=int,
...
base_image_path = job_dir + '/' + args.content_image

style_image_paths = []
for style_image in args.syle_images:
    style_image_paths.append(job_dir + '/' + style_image)
...
def preprocess_image(image_path, load_dims=False, read_mode="color"):
    ...
    img = gsimread(image_path, mode)  # Prevents crashes due to PNG images (ARGB)
...
def load_mask(mask_path, shape, return_mask_img=False):
    ...
    mask = gsimread(mask_path, "L") # Grayscale mask load
...
def create_model(input_tensor, shape, is_vgg19):
    input = Input(tensor=input_tensor, batch_shape=shape)
...
try:
    ...
    if preserve_color:
        content = gsimread(base_image_path, "YCbCr")
    ...
    for i in range(num_iter):
        ...
        try:
            get_file('progress%d.h5' % i, add_query(callback_url, {'step': i + 1}), cache_subdir='models')
        except:
            ex_info = sys.exc_info()
            print("Error reporting the progress:", ex_info)
        ...
        fname = job_dir + "/iteration_%d.jpg" % (i + 1)
        gsimwrite(img, fname)

        print("Image saved as", fname)
        print("Iteration %d completed in %ds" % (i + 1, time.time() - start_time))
except:
    ex_info = sys.exc_info()
    print("The task has exited with an error:", ex_info)
    # Send a task error notification
    get_file('error.h5', add_query(callback_url, {'error': str(ex_info)}), cache_subdir='models')

# Send a task competion notification
get_file('completed.h5', add_query(callback_url, {'gs_url': fname[4:]}), cache_subdir='models')